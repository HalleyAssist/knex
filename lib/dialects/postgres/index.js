  ...
  checkVersion(connection) {
    return new Promise((resolve, reject) => {
      connection.query('select version();', (err, resp) => {
        if (err) {
          // if timecaledb needs update, do it then retry
          if (err.code == '58P01' && err.message.includes('could not access file "$libdir/timescaledb')){
            this.logger.warn('PostgreSQL extension "timescaledb" needs to be updated. Please update it to the latest version.');
            return connection.query('ALTER EXTENSION timescaledb UPDATE;')
              .then(() => connection.query('select version();', (err, resp) => {
