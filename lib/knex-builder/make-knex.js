...
let FunctionHelper, Migrator, Seeder;
...
const KNEX_PROPERTY_DEFINITIONS = {
  ...
  migrate: {
    get() {
      if(!Migrator) Migrator = require('../migrations/migrate/Migrator').Migrator;
  ...
  seed: {
    get() {
      if(!Seeder) Seeder = require('../migrations/migrate/Migrator');
  ...
  fn: {
    get() {
      if(!FunctionHelper) FunctionHelper = require('./FunctionHelper');
